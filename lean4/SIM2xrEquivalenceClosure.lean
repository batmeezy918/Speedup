namespace SIM2xrEquivalenceClosure

universe u v w
variable {X : Type u} {Q : Type v} {Y : Type w}

-- The equivalence-faithful quotient model chronicled from
-- sim2xr_equivalence_faithful.jl:  A = Q*Ar*Q' + Qp*Ap*Qp'
-- with quotient map  Phi(x) = Q' x  and unfold  sigma(z) = Q z.
-- On the invariant sector, T = A and Tbar = Ar intertwine exactly:
--   Phi o T = Tbar o Phi
-- and the section is Q' right inverse of Q (exact), with observable
-- factorization giving the closure obligations proved below.
...
theorem reconstructed_iterate
  ...
  intro n q
  calc
    π (iter n T (σ q)) = iter n Tbar (π (σ q)) := by rw [quotient_iterate π T Tbar hT]
    _ = iter n Tbar q := by rw [hσ]
...
theorem exact_quotient_closure
    ...
    (∀ n x, obs (iter n T x) = obsBar (iter n Tbar (π x))) := by
  exact And.intro (quotient_iterate π T Tbar hT)
    (And.intro (reconstructed_iterate π T Tbar σ hσ hT)
               (observable_preserved_iterate π T Tbar obs obsBar hT hobs))

end SIM2xrEquivalenceClosure